...
def create_adjacency_my_list(my_list):
    ...
    for line in my_list:
        ...
        node = node.replace(' -> ', ' ')
        node = node.split(' ')
        adj_my_list.setdefault(node[0],[]) #adj my_list gets the start of the node pair(ie. first num)
        for num in node[1].split(','): #num gets assigned the end node of the pair. Split on comma needed when multiple end nodes
            adj_my_list[node[0]].append(num)
            circuit_max += 1
    #print(adj_my_list)
    #print(circuit_max)
    return adj_my_list, circuit_max

def find_eulerian_cycle(my_list):
    #create adj my_list
    adj_my_list, circuit_max = create_adjacency_my_list(my_list)

    #reduced adj my_list to keep track of traveled edges
    red_adj_my_list = {}
    red_adj_my_list = deepcopy(adj_my_list) #exact copy of dict
    ...
    while len(circuit) != circuit_max:
        if red_adj_my_list[curr_vert] != []: #if neighbor nodes exist
            stack.append(curr_vert)
            pick = randint(0,len(red_adj_my_list[curr_vert])-1) #what is pick?
            temp = deepcopy(curr_vert) #why use deepcopy
            curr_vert = red_adj_my_list[temp][pick]
            red_adj_my_list[temp].remove(curr_vert)
        else:
